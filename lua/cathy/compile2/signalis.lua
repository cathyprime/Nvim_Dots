...
local exited_len               = #"exited abnormally"

local term_string_pos          = string.find(term_string, "%s", 1, true)
local term_string_abnormal_pos = string.find(term_string_abnormal, "%d", 1, true)
...
local what = function (msg)
    return function (duration)
        ...
        return string.format(
            ...
            os.date "%a %b %d %H:%M:%S",
            duration
        ), function (bufnr, ns, line, higroup)
            vim.hl.range(
                bufnr,
                ns,
                higroup,
                { line, term_string_pos - 1 },
                { line, term_string_pos + #msg }
            )
        end
    end
end
    ...
    },
    {
        __index = function (_, code)
            return function (duration)
                ...
                local cb = function (bufnr, ns, line, higroup)
                    vim.hl.range(bufnr, ns, higroup, { line, term_string_pos - 1 }, { line, term_string_pos + exited_len })
                    vim.hl.range(bufnr, ns, higroup, { line, term_string_abnormal_pos - 1 }, { line, term_string_abnormal_pos + math.ceil(math.log(code, 10)) })
                end
                return str, cb
            end
        end
